...
export default class QueryBuilderGroup extends Vue implements QueryBuilderGroupInterface {
  ...
  get comparator(): RuleSetComparator {
    return this.query.comparator;
  }

  set comparator(ruleSetComparator: RuleSetComparator) {
    this.$emit(
      'query-update',
      {
        ...this.query,
  ...
  get groupOperatorSlotProps(): GroupOperatorSlotProps {
    return {
      currentOperator: this.comparator,
  ...
  addRule(): void {
    ...
    const selectedRule = this.config.rules.find(rule => rule.column === this.selectedRule);
    if (!selectedRule) {
      throw new Error(`Rule column "${this.selectedRule}" is invalid.`);
    }

    if (typeof selectedRule.initialValue === 'object' && selectedRule.initialValue !== null) {
      throw new Error(`"initialValue" of "${selectedRule.column}" must not be an object - use a factory function!`);
    }
    ...
    children.push({
      column: selectedRule.column,
      condition: selectedRule.conditions[0],
      value,
    } as Rule);
  ...
  newGroup(): void {
    ...
    const children = [...this.children];
    children.push({
      comparator: this.config.operators[0].identifier,
      children: [],
    } as RuleSet);
